...
import { useSendChatMessage, useClearChatSession } from "@/src/hooks new/ai-hook";
...
// ← Accept site as a prop
export default function AIChat({ onClose, site }: { onClose: () => void; site: Site }) {
  const [messages, setMessages] = useState<Message[]>([
    {
      id: "1",
      role: "model",
      content: `Hi there! I'm your SproutoGO AI assistant for **${site.name}**. How can I help you with your site today?`,
    },
  ]);
  ...
  return (
    ...
    >
      ...
      <div className="bg-white/5 p-4 flex items-center justify-between border-b border-white/10">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {/* ← Show active site name */}
            <p className="text-xs text-emerald-400/80">{site.name}</p>
          </div>
        </div>
        <button
          onClick={onClose}
      ...
      {/* Messages Area */}
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-transparent flex flex-col justify-between">
        {messages.map((msg) => (
          <div
            key={msg.id}
