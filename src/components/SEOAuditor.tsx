...
import {
  ...
  useReRunAudit,
  useSendChatMessage,
  useDownloadTargetsCsv,
} from "@/src/hooks new/auditor.hook";
...
export default function SEOAuditor({ site }: { site: Site }) {
  ...
  const {
    ...
    refetch,
  } = useGetAudit(site.url);
  const {
    mutate: reRun,
    isPending: isReRunning,
    data: reRunData,
  } = useReRunAudit(site.url);
  const { mutate: sendChat, isPending: isChatLoading } = useSendChatMessage();
  const { mutate: downloadCsv, isPending: isDownloading } =
    useDownloadTargetsCsv();

  const report: AuditData | null =
    reRunData?.data ?? auditResponse?.data ?? null;
  ...
  const organicTrafficValue = Math.floor(
    (totalSearches as unknown as number) * 2.1,
  );
    ...
    try {
      ...
      const kpis = [
        ...
        },
        {
          ...
          value: `${isNaN(organicTrafficValue) ? "—" : organicTrafficValue.toLocaleString()}`,
      ...
      doc.text(
        `${site.name}  ·  GEO/SEO Monthly Report  ·  ${monthName}`,
        14,
        pageHeight - 5,
      );
      doc.setTextColor(16, 185, 129);
      doc.text("Powered by SEO Auditor", pageWidth - 14, pageHeight - 5, {
        align: "right",
      });
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-slate-400 mt-1">
            In-depth breakdown and monthly targets for {site.name}
          </p>
        </div>
        <div className="bg-[#141414] border border-white/10 rounded-2xl p-12 text-center flex flex-col items-center justify-center min-h-[400px]">
          ...
          <h3 className="text-2xl font-bold text-white mb-3">
            Auditor Pending Setup
          </h3>
          <p className="text-slate-400 max-w-md mx-auto leading-relaxed">
            Your GEO/SEO Auditor will become available once your website is live
            and the URL has been connected by our team. Check back soon!
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          >
            <RefreshCw
              className={`w-4 h-4 ${isAnalysing ? "animate-spin" : ""}`}
            />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            {isAnalysing ? (
              ...
              <>
                ...
                <div className="bg-white/5 border border-white/10 p-6 rounded-2xl">
                  ...
                  <div className="text-3xl font-bold text-white mt-4">
                    {isNaN(organicTrafficValue)
                      ? "—"
                      : organicTrafficValue.toLocaleString()}
                  </div>
                  <p className="text-sm text-emerald-400 mt-2 flex items-center gap-1">
            ...
            {isAnalysing ? (
              <div className="flex flex-wrap gap-2">
                {[...Array(6)].map((_, i) => (
                  <div
                    key={i}
                    className="h-8 w-32 rounded-lg bg-white/5 animate-pulse"
                  />
                ))}
              </div>
            ) : (
              <div className="flex flex-wrap gap-2">
                {(report?.trendingKeywords ?? []).map((word, i) => (
                  <span
                    key={i}
                    className="px-3 py-1.5 bg-white/5 border border-white/10 rounded-lg text-sm text-slate-300 flex items-center gap-2"
                  >
        ...
        <div className="bg-white/5 border border-white/10 rounded-2xl flex flex-col h-[800px] lg:h-auto">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 flex flex-col justify-between">
            ...
            <button
              className="text-sm text-slate-400 hover:text-white transition-colors self-start"
              onClick={() =>
                setMessages([
                  {
                    role: "model",
