...
export default function Dashboard({ site }: { site: Site }) {
  ...
  useEffect(() => {
    if (
      aiStats?.totalSearches !== undefined &&
      aiStats?.totalSearches !== null
    ) {
  ...
  return (
    <div className="space-y-6">
      ...
      >
        <div className="p-6 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
          <div className="flex items-center gap-3">
            <div
              className={`w-10 h-10 rounded-xl flex items-center justify-center ${hasUrl ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 shadow-[0_0_15px_rgba(16,185,129,0.2)]" : "bg-white/5 text-slate-500 border border-white/10"}`}
            >
              {hasUrl ? (
            ...
            <div>
              ...
              <p className="text-sm text-slate-400">
                {site.url ?? "Awaiting developer to publish the site."}
              </p>
            </div>
          </div>
          {hasUrl && (
            ...
            <div className="text-center space-y-4">
              ...
              <div>
                ...
                <p className="text-sm text-slate-500 mt-1 max-w-sm mx-auto">
                  Your site preview will appear here once the development team
                  marks it as live.
                </p>
              </div>
            </div>
      ...
      {aiError && (
        ...
        >
          Unable to fetch Google data. Please ensure Google is connected in
          settings.
        </motion.div>
      )}
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center justify-between relative z-10">
                <div
                  className={`w-12 h-12 rounded-2xl flex items-center justify-center ${stat.bg} ${stat.color} border ${stat.border}`}
                >
              ...
              <div className="relative w-full h-full">
                ...
                <div className="absolute top-3 right-3 z-10 flex flex-col gap-2">
                  <button
                    ...
                    className="w-10 h-10 rounded-xl bg-black/70 text-white border border-white/10 hover:bg-black"
                  >
                    +
                  </button>
        ...
        <div className="space-y-4 relative z-10">
          {aiLoading
            ? Array.from({ length: 2 }).map((_, i) => (
                <div
                  key={i}
                  className="h-[72px] rounded-2xl bg-white/5 border border-white/10 animate-pulse"
                />
              ))
            : activities.map((activity, i) => {
                const style =
                  ACTIVITY_STYLES[activity.type] ?? ACTIVITY_STYLES.info;
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 ${style.bg} ${style.color}`}
                    >
